...
def updateCellXY(CL,XYMatrix,XYType="TE"):
...
def GtoJson(G1,prRes):
    ...
    xpca2_matrix=prRes.obsm['X_pca'][:,[0,1]]
    CL=updateCellXY(CL,xpca2_matrix,"PE")
    
    # tsne
    print("UMAP...")

    xtsne_matrix=prRes.obsm['X_umap']
    
    CL=updateCellXY(CL,xtsne_matrix,"TE")
    
    # diffusionmap
    print("diffusion map...")
    xdiff_matrix=prRes.obsm['X_diffmap']
    CL=updateCellXY(CL,xdiff_matrix,"ME") # Coordinates list (e.g., TSNE)
    
    #----------------------------------------------------------------    
    NL=[] # Node List
    
    MTL=[item.mT for item in G1.Nodes]
    MAX_MTL=max(MTL)
    ...
    for j in G1.Edges:
        ...
        EL.append(je)
    GL=[item.upper() for item in GL]  # GeneList (all uppercase)
    
    out=[GL,CL,rootID,NL,EL,dTD]
...
(function(scviz, $, undefined){
    ...
    scviz.onload=function(){
        RL = parseJSON(data);
        nodes = scviz.buildTree(RL[3]);
        rootID = RL[2]; 
        root=nodes.filter(function(d){return d.ID==rootID})[0];
        GL = RL[0];    // list of genes
        cells = RL[1]; // list of cells
        ctypes=cells.map(function(d){return d.typeLabel;});
        utypes=[]
        for (var ct of ctypes){
            if (utypes.indexOf(ct)==-1){
                utypes.push(ct);
            }
        }
        edges = RL[4]; // list of edges
        dTD = RL[5];
        scviz.drawTree(root);
        updateNodes(nodes);
        //alert("done");
    ...
    function inNode(){
        ...
        tooltipsvg.selectAll("path")
            ...
            .attr("fill",function(d,i) { 
                colorDex=utypes.indexOf(d[0]);
                colorDex=colorDex % colorList.length;
                var colorChoosen=colorList[colorDex];
                return colorChoosen;}
                )
    ...
    function parseJSON(data){
        ...
        var EdgeList=data[4];
        var dTD=data[5];    
        return [GL,CellList,rootNode,NodeList,EdgeList,dTD];
    };
